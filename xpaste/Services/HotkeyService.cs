...
public class HotkeyService : IDisposable
{
    ...
    private const uint VK_OEM_PLUS  = 0xBB; // the +/= key (Alt+Shift makes it toggle open)
    private const uint VK_OEM_MINUS = 0xBD; // the -/_ key (Alt+Shift makes it minimize)
    ...
    /// <summary>Fired when <c>Alt+Shift++</c> is pressed to toggle the management window.</summary>
    public event Action? ToggleActivated;

    /// <summary>Fired when <c>Alt+Shift+-</c> is pressed to minimize the app to the tray.</summary>
    ...
    public void Register()
    {
        ...
        bool ok = RegisterHotKey(IntPtr.Zero, ID_TOGGLE, MOD_ALT | MOD_SHIFT | MOD_NOREPEAT, VK_OEM_PLUS);
        AppLogger.Info($"RegisterHotKey toggle ({Hotkeys.ModifierLabel}+Plus): {(ok ? "OK" : $"FAILED err={Marshal.GetLastWin32Error()}")}");
        ok = RegisterHotKey(IntPtr.Zero, ID_MINIMIZE, MOD_ALT | MOD_SHIFT | MOD_NOREPEAT, VK_OEM_MINUS);
        AppLogger.Info($"RegisterHotKey minimize ({Hotkeys.ModifierLabel}+Minus): {(ok ? "OK" : $"FAILED err={Marshal.GetLastWin32Error()}")}");
        for (int i = 0; i < 10; i++)
        {
            ok = RegisterHotKey(IntPtr.Zero, 101 + i, MOD_ALT | MOD_SHIFT | MOD_NOREPEAT, SlotVKeys[i]);
